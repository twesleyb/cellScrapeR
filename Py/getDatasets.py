...
def getDatasets():
    ...
    all_json = [page.find_all('a', href = regex) for page in soup]
    ...
    all_json = [json for (json, empty) in zip(all_json,empty) if not empty]
    # Get dataset names.
    ...
    json_urls = [[BASEURL + link.get('href') for link in links] for links in all_json]
